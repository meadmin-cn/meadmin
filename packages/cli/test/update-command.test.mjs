...
test('命令级：本机registry下载校验，dry-run不写项目，非交互拒绝覆盖',async()=>{
 const root=mkdtempSync(join(tmpdir(),'meadmin-command-'));mkdirSync(join(root,'node_modules/@meadmin/cli'),{recursive:true});
 writeFileSync(join(root,'package.json'),JSON.stringify({name:'business',dependencies:{'@meadmin/cli':'1.0.0'}}));writeFileSync(join(root,'node_modules/@meadmin/cli/package.json'),JSON.stringify({name:'@meadmin/cli',version:'1.0.0',main:'index.js'}));writeFileSync(join(root,'node_modules/@meadmin/cli/index.js'),'');writeFileSync(join(root,'app.ts'),'old');
 const archives=Object.fromEntries(['1.0.0','1.1.0'].map(v=>[v,pack({'package.json':JSON.stringify({name:'create-meadminjs',version:v}),'template/meadmin/packageTemplate.json':JSON.stringify({name:'meadmin',dependencies:{'@meadmin/cli':v}}),'template/meadmin/app.ts':v==='1.0.0'?'old':'new','template/meadmin/meadmin.sql':"CREATE TABLE t(id text PRIMARY KEY); INSERT INTO t(id) VALUES ('x');"})]));
