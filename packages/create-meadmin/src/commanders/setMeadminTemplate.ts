    ...
    Object.keys(jsonObj.devDependencies).forEach((key) => {
      if (jsonObj.devDependencies[key] === "workspace:^") {
        const { version } = JSON.parse(
          readFileSync(
            resolve(fromPath, "node_modules/", key + "/package.json")
          ).toString()
        );
    ...
    .action(async (file: string) => {
      ...
      await Promise.all(
        Object.keys(copyFiles).map(async (key) => {
          if (key.endsWith("/")) {
            await copyPath(
              resolve(fromPath, key),
              resolve(toPath, key),
              "",
              copyFiles[key].ignore || [],
              copyFiles[key].fileSetFunction,
              true
            );
          } else {
            await copyFile(
              resolve(fromPath, key),
              resolve(toPath, key),
              typeof copyFiles[key] === "function" ? copyFiles[key] : undefined,
              true
            );
          }
        })
