...
export class SystemRoleService {
  ...
  async list(queryDto: SystemRoleQueryDto) {
    const where = {} as WhereAttributeHash<InferAttributes<SystemRole, { omit: never }>>;
    (Object.keys(queryDto) as Array<keyof SystemRoleQueryDto>).forEach((key) => {
      ...
      if (key === 'startCreatedAt') {
        where['createdAt'] = (where['createdAt'] ?? {}) as WhereOperators<SystemRole['createdAt']>;
      ...
      if (key === 'startUpdatedAt') {
        where['updatedAt'] = (where['updatedAt'] ?? {}) as WhereOperators<SystemRole['updatedAt']>;
