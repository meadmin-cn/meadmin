...
const normalizePath = (id: string) =>
  path.posix.normalize(isWindows ? id.replace(/\\/g, '/') : id);
interface codeTemplate {
  key: string; //标识符
  template: string; //模板 codeTemplate.template里的{{name}}会被替换为name {{fileName}} 替换为原始文件名 {{path}} 替换为相对路径
...
export const getName = function (
  ...
  const fileNameArr = nameTemplate
...
//获取导入文件绝对地址
export const getFileImportPath = function (
  dir: string,
  fileName: string,
): string {
...
export const getCode = function (
  ...
  fileName: string,
  toFile: string,
  name: string | ((name: string) => string) | undefined = undefined,
  codeTemplates: codeTemplate[] = [],
): codeTemplate[] {
  const filePath = getFileImportPath(dir, fileName);
  let relativePath = normalizePath(
    path.relative(path.dirname(path.resolve(toFile)), filePath),
  );
  if (!relativePath.startsWith('../')) {
    relativePath = './' + relativePath;
  }
  const upName = getName(fileName, name);
  const codeTemplate: codeTemplate[] = JSON.parse(
  ...
  codeTemplate.forEach((item) => {
    item.value = item.template
      .replace(/\{\{name\}\}/g, upName)
      .replace(new RegExp('{{path}}', 'g'), relativePath)
...
export const createPlugin = () => {
  let isServer = false;
  let hasHmr = false;
  const loadFiles: { fileName: string; name: string; dir: string }[][] = [];
  ...
  const loadPath = async function (
    ...
    options: fg.Options,
    name: undefined | string | ((name: string) => string) = undefined,
    template = '',
    codeTemplates: codeTemplate[] = [],
  ) {
    const entries = await fg(
      pattern,
      Object.assign({ cwd: dir, dot: true }, options),
    );
    let str = template
      ? template
  ...
  function resolver(
    componentInclude: number[],
    directiveInclude: number[] = [],
  ): any[] {
    const result = [];
      ...
      result.push({
        ...
        resolve: async (componentName: string) => {
          for (const index of componentInclude) {
            const componentInfo = loadFiles[index].find(
              ({ name }) => name == componentName,
            );
            if (componentInfo) {
              return normalizePath(
                path.resolve(componentInfo.dir, componentInfo.fileName),
              );
            }
          }
        },
      ...
      result.push({
        ...
        resolve: async (directiveName: string) => {
          for (const index of directiveInclude) {
            const directiveInfo = loadFiles[index].find(
              ({ name }) =>
                name == directiveName || name == 'V' + directiveName,
            );
            if (directiveInfo) {
              return normalizePath(
                path.resolve(directiveInfo.dir, directiveInfo.fileName),
              );
            }
          }
        },
  ...
  class loadPathsPlugin {
    constructor(public dirOptions: DirOptions) {}

    hmr() {
      this.dirOptions.forEach((item, index) => {
        isServer = true;
        fs.watch(
          item.dir,
          { recursive: true },
          function (eventType: fs.WatchEventType, fileName: string) {
            ...
            if (eventType === 'rename') {
              ...
              const filePath = path.resolve(item.dir, fileName);
              if (fs.existsSync(filePath)) {
                ...
                if (stat.isFile()) {
                  ...
                  tmpRemovePaths.length &&
                  fs.existsSync(path.resolve(filePath, tmpRemovePaths[0]))
                ) {
                  //如果是重命名文件夹
                ...
                loadFiles[index].slice(0).forEach((val, k) => {
                  if (
                    val.fileName.startsWith(fileName + '/') ||
                ...
                if (changeFiles.length) {
                  ...
                  if (changeFiles[0] !== fileName) {
                    //代表是文件夹改变
                    tmpRemovePaths = changeFiles.map((name) =>
                      name.slice(fileName.length + 1),
                    );
                  } else {
                    tmpRemovePaths = [];
                  }
    ...
    async build() {
      ...
      if (isServer) {
        this.dirOptions.forEach((item) => {
          toFileContents.set(
            item.toFile,
            readFileSync(item.toFile, 'utf8') as string,
          );
          fs.watch(
            item.toFile,
            {},
            function (eventType: fs.WatchEventType, fileName: string) {
              ...
              if (
                content !== undefined &&
                content !== readFileSync(item.toFile, 'utf8')
              ) {
    ...
    apply(compiler: any) {
      const pluginName = loadPathsPlugin.name;
      compiler.hooks.watchRun.tapAsync(
        { name: pluginName },
        (compiler: any, callback: () => void) => {
          if (hasHmr) {
            return callback();
          }
          hasHmr = true;
