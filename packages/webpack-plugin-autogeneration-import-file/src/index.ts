...
const normalizePath = (id: string) => path.posix.normalize(isWindows ? id.replace(/\\/g, '/') : id);
...
export const getCode = function (dir: string,
    ...
    codeTemplate.forEach((item) => {
        item.value = item.template.replace(/\{\{name\}\}/g, upName).replace(new RegExp('\{\{path\}\}', 'g'), relativePath).replace(new RegExp('\{\{fileName\}\}', 'g'), fileName);
    })
    return codeTemplate;
}
