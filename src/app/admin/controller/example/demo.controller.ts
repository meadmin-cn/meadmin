...
export class ExampleDemoController extends BaseController {
  ...
  async getUser(
    ...
    return this.success(
      await this.exampleDemoService.getUser(page, pageSize, id, username)
    );
  }
  ...
  async getExampleBook(
    ...
    return this.success(
      await this.exampleDemoService.getExampleBook(page, pageSize, id, name)
    );
  }
  ...
  async update(
    @Param("id") id: string,
    @Body() updateDto: ExampleDemoUpdateDto
  ) {
