...
//基础model
@Table.Abstract
export class BaseModel<M extends Model<any, any>> extends Model<
  InferAttributes<M>,
...
> {
  ...
  @BeforeCreate()
  static async setCreatedId(
    info: BaseModel<any>,
    options: InstanceUpdateOptions<BaseModel<any>>
  ) {
    let ctx: Context;
    try {
      ctx = getContext();
    ...
    if (
      ctx?.adminInfo &&
      info.modelDefinition.attributes.has("createdAdminId")
    ) {
      //设置创建管理员Id
      (info as any).createdAdminId = ctx.adminInfo.id;
      if (
        ctx?.adminInfo &&
        info.modelDefinition.attributes.has("updatedAdminId")
      ) {
        //设置更新管理员Id
    ...
    if (ctx?.userInfo && info.modelDefinition.attributes.has("createdUserId")) {
      ...
      if (
        ctx?.userInfo &&
        info.modelDefinition.attributes.has("updatedUserId")
      ) {
        //设置更新用户Id
  ...
  static async setCreatedIdBulk(
    ...
    if (
      ctx?.adminInfo &&
      options.model.modelDefinition.attributes.has("createdAdminId")
    ) {
      //设置创建管理员Id
    ...
    if (
      ctx?.userInfo &&
      options.model.modelDefinition.attributes.has("createdUserId")
    ) {
      //设置创建用户Id
      instances.forEach((instance) => {
  ...
  static async setUpdatedIdBulk(
    ...
    if (
      ctx?.adminInfo &&
      options.model.modelDefinition.attributes.has("updatedAdminId")
    ) {
      //设置更新管理员Id
    ...
    if (
      ctx?.userInfo &&
      options.model.modelDefinition.attributes.has("updatedUserId")
    ) {
      //设置更新用户Id
      await options.model.update(
