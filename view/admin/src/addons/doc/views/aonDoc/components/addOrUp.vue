      ...
      <template v-if="info.type == 2">
        <el-form-item :label="t('内容类型')" prop="constentType">
          <el-select v-model="info.constentType" :value-on-clear="null" clearable>
            <el-option v-for="val in dict.constentType" :key="val.value" :value="val.value" :label="val.label" />
          </el-select>
        </el-form-item>
        <el-form-item v-if="info.constentType == 0" :label="t('内容')" prop="mdContent">
          <aon-doc-md-editor v-model="info.mdContent"></aon-doc-md-editor>
        </el-form-item>
        <el-form-item v-if="info.constentType == 1" :label="t('外链地址')" prop="link">
