...
//执行分片上传
const uploadChunksExecute = (
  file: UploadRawFile,
  currentChunk: number,
  chunkSize: number,
  data: UploadRequestOptions["data"] = {}
) =>
    ...
    try {
      ...
      fileReader.onload = async (e) => {
        try {
          ...
          data.start = start + "";
          data.chunkMd5 = SparkMD5.ArrayBuffer.hash(
            e.target!.result as ArrayBuffer
          );
          Object.keys(data).forEach((key) => {
            formData.append(
              key,
              Array.isArray(data[key]) ? String(data[key]) : data[key]
            );
          });
...
const uploadChunks = (
  ...
  new Promise<Partial<FileInfo>>(async (resolve, reject) => {
    try {
      ...
      if (chunks > 1) {
        //先上传一个分片确保校验和秒传
        const res = await uploadChunksExecute(
          file,
          currentChunk,
          chunkSize,
          Object.assign(data, { chunkIndex: currentChunk + "", over: "0" })
        );
        if (res.id) {
          progress?.("100");
          return resolve(res);
      ...
      const res = await uploadChunksExecute(
        ...
        currentChunk,
        chunkSize,
        Object.assign(data, { chunkIndex: currentChunk + "", over: "1" })
      );
      progress?.("100");
      return resolve(res);
    } catch (e) {
