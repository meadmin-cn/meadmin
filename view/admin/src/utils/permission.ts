...
export type DynamicViewsModules = Record<
  string,
...
export function initDynamicViewsModules() {
  if (!dynamicViewsModules) {
    dynamicViewsModules = import.meta.glob("../views/**/*.{vue,tsx}") as Record<
      string,
      () => Promise<DefineComponent>
    >;
    dynamicAddonsViewsModules = import.meta.glob(
      "../addons/*/views/**/*.{vue,tsx}"
...
export function transitionComponent(component: string) {
  ...
  if (component.startsWith("addons")) {
    res =
      dynamicAddonsViewsModules["../" + component + ".vue"] ||
      dynamicAddonsViewsModules["../" + component + ".tsx"];
  } else {
    res =
      dynamicViewsModules["../views/" + component + ".vue"] ||
      dynamicViewsModules["../views/" + component + ".tsx"];
  }
  if (res) {
    return res;
  } else {
