<template>
  <el-card shadow="hover" :header="t('浏览量')" class="card-item">
    <el-skeleton :rows="8" :loading="false">
...
await loadRes;
echarts.use([TitleComponent, ToolboxComponent, TooltipComponent, GridComponent, LegendComponent, LineChart, PieChart, RadarChart, SVGRenderer, UniversalTransition]);
...
watchPostEffect(() => {
  ...
  if (data) {
    ...
    allocationOptions.series[0].data.forEach((item, key) => {
      item.value = data.allocation[key];
    });
