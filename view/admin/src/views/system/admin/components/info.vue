...
<me-dialog  v-model="show" :title="t('详情')" :close-on-click-modal="false" @closed="$emit('closed')">
  <el-descriptions class="info" :border="true" v-loading="loading">
    ...
    <el-descriptions-item :label="t('创建人')">
      {{formatterStrExec(data?.createdAdmin?.nickname)}}({{ formatterStrExec(data?.createdAdmin?.username) }})
    </el-descriptions-item>
    <el-descriptions-item :label="t('最后更新人')">
      {{formatterStrExec(data?.updatedAdmin?.nickname)}}({{ formatterStrExec(data?.updatedAdmin?.username) }})
    </el-descriptions-item>
  </el-descriptions>
  <template #footer>
...
import { formatterStrExec,formatterDictExec,formatterAtExec } from '@/utils/helper.js';
