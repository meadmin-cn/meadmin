<template>
  <me-dialog v-model="show" :title="t('详情')" :close-on-click-modal="false" @closed="emit('closed')">
    <el-descriptions v-loading="loading" class="info" :border="true">
      ...
      <el-descriptions-item :label="t('创建者(用户)')"> {{ formatterObjectExecFn((obj) => `${obj.nickname}(${obj.username})`)(data?.createdUser) }} </el-descriptions-item>
      <el-descriptions-item :label="t('最后更新者(用户)')"> {{ formatterObjectExecFn((obj) => `${obj.nickname}(${obj.username})`)(data?.updatedUser) }} </el-descriptions-item>
    </el-descriptions>
    <template #footer>
...
import { userFileInfoApi } from '@/api/userFile';
...
const { data, loading, runAsync } = userFileInfoApi();
watch(
  () => props.id,
  async (id?: string) => {
    if (id) {
