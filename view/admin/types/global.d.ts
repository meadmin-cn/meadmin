...
//_或-间隔转换为驼峰
type CamelCase<S extends string> = S extends `${infer First}_${infer Second}${infer Rest}` ? `${First}${Uppercase<Second>}${CamelCase<Rest>}` : S extends `${infer First}-${infer Second}${infer Rest}` ? `${First}${Uppercase<Second>}${CamelCase<Rest>}` : S;
